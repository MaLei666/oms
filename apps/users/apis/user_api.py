...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if request.user.role < role :
            if role<4 \
                    or (role==4 and (request.user.unit_id==request.data.get('unit_id') or request.user.role<3)) \
                    or (role==5 and (request.user.dept_id==request.data.get('dept_id') or request.user.role<3))\
                    or (role==5 and request.user.unit_id==request.data.get('unit_id') and request.user.role==3):
                serializer = self.get_serializer(data=request.data)
                serializer.is_valid(raise_exception=True)
                self.perform_create(serializer)
                return Response(self.code.request_add_succeed())
